...
public partial class FormProducts : Form
{
	...
	private async void Btn_Cadastrar_Produto_Click(object sender, EventArgs e)
	{
		...
		else
		{
			...
			HttpResponseMessage? httpResponse = await HttpClientProducts.DoPost(request);
	...
	{		
		...
		{
			ResponseProduct? productResponse = await HttpClientProducts.DoGetByName(txt_Name.Text);
		...
		{
			MessageBox.Show("Infome o nome do product.","Produtos", MessageBoxButtons.OK,MessageBoxIcon.Information);
		}
	}
	...
	private async void Btn_Editar_Click(object sender, EventArgs e)
	{
		...
		bool isSuccessfull = await HttpClientProducts.DoPut(id, request);
	...
	private async void Btn_Deletar_Click(object sender, EventArgs e)
	{
		...
		if(id != 0)
		{
			string message = await HttpClientProducts.DoDelete(id);
